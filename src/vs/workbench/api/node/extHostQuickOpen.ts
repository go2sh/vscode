...
export class ExtHostQuickOpen implements ExtHostQuickOpenShape {
	...
	$onDidFocusChange(sessionId: number, handles: number[]): void {
		const session = this._sessions.get(sessionId);
		if (session instanceof ExtHostQuickPick) {
			session._fireDidFocusChange(handles);
		}
	}

	$onDidSelectionChange(sessionId: number, handles: number[]): void {
		const session = this._sessions.get(sessionId);
		if (session instanceof ExtHostQuickPick) {
